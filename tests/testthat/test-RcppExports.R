...
test_that("`distance_ls_cpp()` works", {
  x <- zoo_simulate(seed = 1, irregular = FALSE)
  y <- zoo_simulate(seed = 2, irregular = FALSE)
  dist_matrix <- distance_ls_cpp(x = x, y = y, distance = "euclidean")
})
...
test_that("`importance_dtw_cpp()` works", {
  x <- zoo_simulate(seed = 1, rows = 100)
  y <- zoo_simulate(seed = 2, rows = 150)
  expect_equal(nrow(x) == nrow(y), )
  df <- importance_dtw_cpp(x = x, y = y, distance = "euclidean")
  expect_equal(df, )
})

test_that("`importance_dtw_legacy_cpp()` works", {
  x <- zoo_simulate(seed = 1, rows = 100)
  y <- zoo_simulate(seed = 2, rows = 150)
  expect_equal(nrow(x) == nrow(y), )
  df <- importance_dtw_legacy_cpp(x = x, y = y, distance = "euclidean")
  expect_equal(df, )
})

test_that("`importance_ls_cpp()` works", {
  x <- zoo_simulate(seed = 1, irregular = FALSE)
  y <- zoo_simulate(seed = 2, irregular = FALSE)
  expect_equal(nrow(x) == nrow(y), )
  df <- importance_ls_cpp(x = x, y = y, distance = "euclidean")
  expect_equal(df, )
})
