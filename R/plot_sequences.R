...
#' @param x_subset (optional, integer or character vector) Vector of numeric indices or names of `x` to plot. Default: NULL
...
plot_sequences <- function(
  ...
  if(!is.null(x_subset)){
    ...
    if(is.numeric(x_subset)){
      ...
      x_subset <- as.integer(x_subset)

      if(max(x_subset) > length(x)){
        x_subset[x_subset > length(x)] <- length(x)
      }

      if(min(x_subset) < 1){
        x_subset[x_subset < 1] <- 1
      }

      x_subset <- unique(x_subset)

    }

    #using names
    if(is.character(x_subset)){

      x_subset <- x_subset[x_subset %in% names(x)]
