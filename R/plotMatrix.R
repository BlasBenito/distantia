...
#' @param legend boolean. If \code{TRUE}, the plot is made with \code{\link[fields]{image.plot}}, and includes a color scale on the right side. If \code{FALSE}, the plot is made with \code{\link[base]{image}}, and the color scale is omitted.
...
plotMatrix <- function(distance.matrix = NULL,
  ...
  for(i in 1:n.elements){
    ...
    #add path
    if(!is.null(least.cost.path)){
       lines(path.i$A, path.i$B, lwd=path.width, col = path.color)
    }
