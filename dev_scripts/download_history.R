# Install packages if needed
# install.packages(c("cranlogs", "ggplot2"))

library(cranlogs)
library(ggplot2)


# Get download data from the beginning
distantia_downloads <- cran_downloads(
  packages = "distantia",
  from = "2019-06-01",  # Earliest available data
  to = Sys.Date()
)

# Optional: Monthly aggregation for cleaner visualization
distantia_downloads$year_month <- format(distantia_downloads$date, "%Y-%m")
monthly_downloads <- aggregate(
  count ~ year_month,
  data = distantia_downloads,
  FUN = sum
)
monthly_downloads$date <- as.Date(paste0(monthly_downloads$year_month, "-01"))

# Calculate average monthly downloads
avg_monthly <- mean(monthly_downloads$count)

# Version 2.0.0 release date
release_date <- as.Date("2025-01-01")

# Plot monthly downloads
ggplot(monthly_downloads, aes(x = date, y = count)) +
  geom_col(fill = "#3498DB", alpha = 0.8) +
  geom_hline(
    yintercept = avg_monthly,
    linetype = "dashed",
    color = "#E74C3C",
    linewidth = 0.8
  ) +
  geom_vline(
    xintercept = release_date,
    linetype = "solid",
    color = "gray20",
    linewidth = 0.8
  ) +
  annotate(
    "text",
    x = max(monthly_downloads$date),
    y = avg_monthly,
    label = paste0("Avg: ", round(avg_monthly)),
    vjust = -0.5,
    hjust = 1,
    color = "#E74C3C",
    size = 3.5
  ) +
  annotate(
    "text",
    x = release_date,
    y = max(monthly_downloads$count),
    label = "v2.0.0",
    vjust = -0.5,
    hjust = -0.1,
    color = "gray20",
    size = 3.5,
    fontface = "bold"
  ) +
  theme_minimal() +
  labs(
    title = "distantia Package Download History (Monthly)",
    subtitle = paste0("Total downloads: ", format(sum(monthly_downloads$count), big.mark = ",")),
    x = "Month",
    y = "Monthly Downloads",
    caption = "Data source: RStudio CRAN mirror via cranlogs"
  ) +
  theme(
    plot.title = element_text(face = "bold", size = 14),
    plot.subtitle = element_text(size = 11, color = "gray40"),
    panel.grid.minor = element_blank()
  )
