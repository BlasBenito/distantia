...
The code below loads the required packages, defines a parallelization backend and loads the example data.
...
The `albatross` data frame contains the flight paths of 4 individuals of Waved Albatross captured during summer of 2008.

```{r, echo = FALSE}
head(distantia::utils_drop_geometry(df = albatross))
```
```{r}
