...
The functions to manage TSLs **are a bunch of `lapply` in a trench coat**. Most use `future.apply::future_lapply()`, a parallelized version of `lapply` from the `future.apply` package, and a few combine `foreach::foreach()` with `doFuture::%dofuture%` in shameless parallelized loops.
...
#### Compositional Transformations
  ...
  - Percentage (`f_percentage`): Suitable for count or continuous data when values need to be expressed as percentages of the total. Commonly used in relative abundance or proportional datasets.
  - Proportion (`f_prooportion`): Best for count or continuous data where the goal is to normalize values to a unit sum, ensuring they represent fractions of the total.
  - Square Root of Proportion (`f_proportion_sqrt`): Ideal for proportional or count data to reduce the influence of dominant values and emphasize smaller contributions.
  - Hellinger (`f_hellinger`): Typically applied to count data (e.g., species abundance) to compute distances that are symmetric and non-linear, enhancing interpretability in ecological analyses.
  - Centered Log-Ratio (`f_clr`): Designed for compositional data (proportions or relative abundances) to address the unit-sum constraint and emphasize relative changes between components.
  - Chi-Squared (`f_chi_square`): Suitable for count data, especially when comparing observed and expected frequencies in categorical or proportional datasets.
  - Log Transformation  (`f_log`): Applicable to count or continuous data with positive values, often used to reduce skewness and stabilize variance in datasets with exponential growth or wide value ranges.
...
distantia::tsl_plot(tsl = tsl[1:3])
```
...
tsl_plot(tsl = tsl_hellinger[1:3])

tsl_plot(tsl = tsl_hellinger[6])
```


```{r, fig.height=5}
distantia::distantia_plot(
  tsl = tsl_hellinger[c(1, 6)]
)
```
