...
For example, the dataset `fagus_dynamics` has variables in different units, and is a good example for this topic.
...
```
Due to the differences in magnitude between variables, a dynamic time warping analysis will focus on `rainfall` disproportionately, biasing the results. The code below computes the overall contribution of each variable to the dissimilarity between locations to show this issue.

```{r, fig.height=2.5}
df_importance <- distantia::distantia_importance(
  tsl = tsl
)
...
```
The boxplot indicates that `rainfall` is the only variable contributing to similarity because its higher values take over when computing euclidean distances between samples.
...
If we analyze the individual contribution to dissimilarity for both scalings.

```{r, fig.height=2.5}
df_local_scaling <- distantia::distantia_importance(
  tsl = tsl_local_scaling
)

df_global_scaling <- distantia::distantia_importance(
  tsl = tsl_global_scaling
)
