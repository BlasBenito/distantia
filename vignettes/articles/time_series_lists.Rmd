...
The `distantia` package offers transformations for ecological and compositional data, including percentage (`f_percentage`), proportion (`f_proportion`), and square root of proportion (`f_proportion_sqrt`) for normalization and reducing dominance. Hellinger (`f_hellinger`) and centered log-ratio (`f_clr`) enhance interpretability in abundance and compositional datasets. Chi-squared (`f_chi_square`) and log transformation (`f_log`) address variance stabilization and frequency comparisons.

The example below focuses on the pollen counts dataset `eemian_pollen` to showcase several of these transformations.
...
```
The code below applies `f_hellinger`, which transforms the pollen counts to proportions and the applies the Hellinger transformation. This transformation helps mitigate the effects of highly abundant taxa, and improves dissimilarity assessments with `distantia()` and `distantia_importance()`.
