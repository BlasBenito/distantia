...
The R package `distantia` 2.0 introduces Time Series Lists as an object to organize time series for dissimilarity analyses, and provides a complete toolset to manage them.
...
Only `zoo` and `distantia` are required for this tutorial.
...
Other advantages of using zoo objects include seamless integration with base R methods, and built-in tools for alignment, merging, and subsetting.
...
In this case, the column names of the univariate zoo objects will be "x".
...
As such, they all support a parallelization backend provided by the `future` package, as shown below. However, keep in mind that parallelization is only worth it for very large datasets.
...
The second workflow involves the functions `tsl_diagnose()` and `tsl_repair()`, and is more general because it addresses other potential issues at once.
...
To solve this issue, the package `distantia` implements two flavors of scaling and/or centering:
  ...
  - **Global**: each variable is scaled and/or centered using its mean and standard deviation across all time series.
...
Removing trends in time series before applying dynamic time warping may help prevent inflated dissimilarity scores caused by non-stationary components. Detrending also ensures that the alignment generated by dynamic time warping focuses on meaningful shape features rather than being distorted by long-term trends.
...
Centered log-ratio computed with `f_clr`.
