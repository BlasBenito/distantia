...
The functions `f_rescale_local` and `f_rescale_global` work under the same principle to rescale the range of the time series variables. By default, these functions rescale all time series between 0 and 1, but the arguments `new_min` and `new_max` can receive numeric vestor to set different new ranges for each column in a time series list.
...
The stats below show the result of the rescaling for each method.
