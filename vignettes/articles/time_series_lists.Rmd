...
The code below creates a TSL with two irregular time series with different number of rows.
...
```
The time features of this TSL show clear differences in frequency between the time series A and B.

```{r}
distantia::tsl_time(tsl = tsl)[, c("name", "rows", "resolution", "begin", "end")]
```
By default, if the argument `new_time` is omitted, a linear model is used to resample the TSL to its average resolution over the intersection of its time ranges.
...
distantia::tsl_time(tsl = tsl_resampled)[, c("name", "rows", "resolution", "begin", "end")]
...
```
Otherwise, a keyword can be used to define the resampling frequency. Again, we can find valid keywords using `distantia::tsl_time()`, but replacing "aggregate" with "resample" in the argument `keywords`.
...
tsl_time(tsl = tsl_resampled)[, c("name", "rows", "resolution", "begin", "end")]
...
This method computes a statistic over a fixed-width window of consecutive cases and replaces each central value with the computed statistic. It is commonly used to mitigate noise in high-frequency time series. In `distantia()`, this functionality is implemented in `tsl_smooth()`. Please, take in mind that it should not be applied to highly irregular time series, as it ignores time distance.

The code below generates a noisy high-frequency time series, and applies `tsl_smooth()` with a couple of window sizes.
