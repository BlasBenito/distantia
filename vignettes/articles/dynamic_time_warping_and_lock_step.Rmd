...
The R package `distantia` offers two general methods to compare time series: **dynamic time warping** (DTW) and **lock-step** (LS). This article explains their implementation within `distantia` and illustrates their use cases with code examples.  
...
```{r, echo = FALSE}
if(!requireNamespace(package = "dtw", quiet = TRUE)){
...
The code below performs these operations:
  ...
  - Applies `tsl_subset()` to select three cities and a shorter time span to simplify the data.
...
```
The resulting TSL contains two synchronous time series with similar temperature ranges (London and Paris), and a third one (Kinshasa) with a much higher average temperature and a shifted temporal pattern. Using a relative vertical axis in `tsl_plot()` should make these patterns more obvious.
...
Finally, to help compare the results of DTW and LS, the code below applies the functions `tsl_transform()` and `f_scale` to center and scale the data.
...
((2 * step_1) / step_2)
```
In `distantia()`, the lower level implementation of this concept is in the C++ function `psi_lock_step_cpp()`, which calls `distance_lock_step_cpp()`, `auto_sum_full_cpp()`, and `psi_equation_cpp()` to perform steps one to three.
...
```
Finally, the proper user interface to perform this operation is the function `distantia()` with `lock_step = TRUE` (default is `FALSE`).
...
This method aligns the time axes of the compared time series to maximize the match between similar patterns, even if they are shifted in time. This method is particularly useful for comparing time series with similar shapes but temporal misalignments. However, it comes at the cost of higher computational complexity and lower efficiency compared to the lock-step method, and it may be challenging to apply to time series with thousands of data points.

DTW follows the same three conceptual steps as the lock-step method, but with significant additional logic in the first step:

**1.-** Sum the distances between pairs of samples.

  - **1a** Compute distance matrix between all pairs of samples in both time series.

  - **1b** Compute least-cost matrix.

  - **1c** Find least-cost path.

  - **1d** Sum distances of pair of samples connected by the least-cost path.
      
**2.-** Sum the distances between consecutive samples within each time series.
  
**3.-** Compute the normalized dissimilarity score *psi* (with a slightly different formula!).
...
1.- Computation of the distance matrix between all pairs of samples. This step is implemented in the C++ function `distance_matrix_cpp()`. Below we its R interface `psi_distance_matrix()` just because it prepares the matrix for plotting with `utils_matrix_plot()`.

```{r}
m.dist <- distance_matrix_cpp(
  x = x,
  y = y,
  distance = "euclidean"
)
