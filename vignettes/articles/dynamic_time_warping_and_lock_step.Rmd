...
All these individual steps can be performed at once with the function `distantia()` by setting the argument `lock_step` to `FALSE`.
...
These long horizontal and vertical lines shown above provide an immediate diagnostic of the issue. 
...
Itakura parallelograms cannot fix bad alignments (that's what data scaling is for), but can help diagnose them, given a tolerance level.
