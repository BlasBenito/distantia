...
and applies global scaling and centering with
[`tsl_transform()`](https://blasbenito.github.io/distantia/reference/tsl_transform.md)
and `f_scale_global` to facilitate time series comparisons.
...
enables a quick identification of the time series that are either more
dissimilar (top) or similar (bottom) to others.
...
computes unrestricted dynamic time warping with orthogonal and diagonal
least cost paths.
...
provides detailed insights into the alignment between a pair of time
series resulting from DTW.
...
The function
[`distantia()`](https://blasbenito.github.io/distantia/reference/distantia.md)
implements restricted permutation tests to assess the significance of
dissimilarity scores. It provides several setups to support different
assumptions.
...
more to similarity or dissimilarity. The
...
function provides a quick insight into which variables contribute the
most to similarity or dissimilarity across all pairs of time series.
